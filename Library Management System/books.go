...
type Book interface {
	...
	Name() string
	Author() string
	Borrow(string) bool // Borrow accepts a string and borrows
	Return(string)      // returns success of borrow function
}
...
func (d *DigitalBook) Borrow(borrower string) bool {
	if len(d.borrowers) < d.limit { // Check if there is no current borrower
...
//Removes the book with index as an argument
